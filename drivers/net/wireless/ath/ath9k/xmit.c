			      ...
			      struct ath_atx_tid *tid, bool *stop)
{
	...
	printk("ath_tx_sched_aggr tid : %d\n", tid);
				   ...
				   bool more_data)
{
	...
	printk("ath9k_release_buffered_frames\n");
	INIT_LIST_HEAD(&bf_q);
			      ...
			      struct list_head *head, bool internal)
{
	...
	for (ptr = head; ptr->next != head; ptr = ptr->next) printk("txqaddbuf %p %p : %p %p\n", head, ptr, ptr->prev, ptr->next);
...
static void ath_tx_send_normal(struct ath_softc *sc, struct ath_txq *txq,
			       struct ath_atx_tid *tid, struct sk_buff *skb)
{
	if (skb) printk("send_normal %p %p %p %p\n", skb->head, skb->data, skb->tail, skb->end);
